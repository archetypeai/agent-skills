...
| `lns-1d519091822706e2-bc108andqxf8b4os` | `lens_timeseries_state_processor` | `OmegaEncoder::omega_embeddings_01` | **Machine State Lens — n-shot KNN classifier.** Real-time per-window classification with SSE output, hosted KNN over n-shot focus CSVs. Default buffer/window/step = 1024 (much larger than the per-window probes in `/query`), `normalize_input: true`. Used by [`newton-machine-state`](../newton-machine-state/SKILL.md). **Recommended pattern: register a child lens pinned to `omega_embeddings_1_4` rather than using this lens directly** — the pinned `_01` has been observed to silently emit zero `inference.result` events on staging (no error, just heartbeats). See [`newton-machine-state` Staging Gotchas](../newton-machine-state/SKILL.md#staging-gotchas) for the full failure-mode catalog and the [parallel-subsystem pattern](../newton-machine-state/references/parallel-subsystem-pattern.md) for the child-lens setup. |
...
| Streaming per-window classification with a hosted KNN | **Child lens pinned to `OmegaEncoder::omega_embeddings_1_4`** registered via `POST /lens/register` (parent processor: `lens_timeseries_state_processor`). The platform-mounted base lens `lns-1d519091822706e2-…` pins `_01` and has been observed to silently emit zero `inference.result` events end-to-end — see [`newton-machine-state` Staging Gotchas](../newton-machine-state/SKILL.md#staging-gotchas). | Lens session | [`newton-machine-state`](../newton-machine-state/SKILL.md) |
