...
Verified on the [`archetypeai-wind-turbine-demo`](https://github.com/archetypeai/archetypeai-wind-turbine-demo) — same Penmanshiel app that previously hit the allocation error. After switching from two-lens / two-session to one-lens / one-session-with-multiplex, both turbines stream interleaved verdicts (WT01: 34 predictions, WT09: 33 predictions in 80s) with zero allocation errors. The per-stream prediction throughput halves (since both streams share one ~1/s runner), but for visualization workloads that's typically fine. See [`newton_client.MultiplexNewtonSession`](https://github.com/archetypeai/archetypeai-wind-turbine-demo/blob/main/newton_client.py) for the full Python shape (FIFO push-tag queue, channel-first `session.update` payload, per-stream prediction counter).
